 ...
 *
 * **This lives in core rather than in the agent because two callers need it.** The
 * agent runs it inside the Cartographer. The worker runs it on records whose
 * Curator output came back from `curator_cache`: a cached result carries the
 * classification and the records, but not the attributions, because attributions
 * depend on the alias table as it stands at call time and the cache is keyed on
 * message content alone. Without a shared matcher the worker would need a second
 * implementation of this, and two copies would not fail a build or a test — they
 * would silently stop agreeing, and a mention that should have resolved to a
 * person would quietly resolve to nobody.
 */

import { normaliseAlias } from "./normalise.js";
import type { AliasEntry } from "./schemas/tasks.js";
